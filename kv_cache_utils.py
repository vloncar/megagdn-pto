...
def get_uniform_page_size(kv_cache_specs: Iterable[KVCacheSpec]) -> int:
    ...
    """
    specs_list = list(kv_cache_specs)
    page_sizes = {layer.page_size_bytes for layer in specs_list}
    if len(page_sizes) != 1:
        print(f"[DEBUG kv_cache_utils] get_uniform_page_size: non-uniform page sizes {sorted(page_sizes)}")
        for s in specs_list:
            print(f"[DEBUG kv_cache_utils]   spec type={type(s).__name__} page_size={s.page_size_bytes} block_size={s.block_size}")
    assert len(page_sizes) == 1, f"Non-uniform page sizes across KV cache groups: {sorted(page_sizes)}"
...
def unify_kv_cache_spec_page_size(
    ...
    print(f"[DEBUG kv_cache_utils] unify_kv_cache_spec_page_size: {len(page_sizes)} distinct page sizes: {sorted(page_sizes)}")
    for layer_name, layer_spec in kv_cache_spec.items():
        print(f"[DEBUG kv_cache_utils]   {layer_name}: type={type(layer_spec).__name__} "
              f"page_size={layer_spec.page_size_bytes} block_size={layer_spec.block_size} "
              f"real_page_size={getattr(layer_spec, 'real_page_size_bytes', None)}")
        ...
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if max_page_size % layer_page_size != 0:
                raise NotImplementedError(
                    f"Cannot unify page sizes: layer '{layer_name}' has page_size={layer_page_size}, "
                    f"max_page_size={max_page_size}, remainder={max_page_size % layer_page_size}. "
                    f"All page sizes: {sorted(page_sizes)}"
                )
            ratio = max_page_size // layer_page_size
            new_block_size = layer_spec.block_size * ratio
            new_spec = replace(layer_spec, block_size=new_block_size)
            assert new_spec.page_size_bytes == max_page_size, (
                f"Page size mismatch after adjustment: {new_spec.page_size_bytes} != {max_page_size}"
            )
...
def get_kv_cache_groups(
    ...
    """
    print(f"[DEBUG kv_cache_utils] get_kv_cache_groups: {len(kv_cache_spec)} layers")
    type_counts: dict[str, int] = {}
    for layer_name, spec in kv_cache_spec.items():
        t = type(spec).__name__
        type_counts[t] = type_counts.get(t, 0) + 1
    print(f"[DEBUG kv_cache_utils]   spec types: {type_counts}")
    page_sizes = sorted({s.page_size_bytes for s in kv_cache_spec.values()})
    print(f"[DEBUG kv_cache_utils]   distinct page sizes: {page_sizes}")
    for layer_name, spec in kv_cache_spec.items():
        print(f"[DEBUG kv_cache_utils]   {layer_name}: {type(spec).__name__} "
              f"page={spec.page_size_bytes} block_size={spec.block_size} "
              f"real_page={getattr(spec, 'real_page_size_bytes', None)}"
              + (f" shapes={getattr(spec,'shapes',None)} mamba_type={getattr(spec,'mamba_type',None)}"
                 if hasattr(spec, 'shapes') else ""))
...
def get_kv_cache_configs(
    ...
    print(f"[DEBUG kv_cache_utils] get_kv_cache_configs: {len(kv_cache_specs)} workers, "
          f"available_memory={[f'{m//1024//1024} MiB' for m in available_memory]}")
    for i, worker_spec in enumerate(kv_cache_specs):
        type_counts: dict[str, int] = {}
        for spec in worker_spec.values():
            t = type(spec).__name__
            type_counts[t] = type_counts.get(t, 0) + 1
        page_sizes = sorted({s.page_size_bytes for s in worker_spec.values()})
        print(f"[DEBUG kv_cache_utils]   worker[{i}]: {len(worker_spec)} layers, "
              f"types={type_counts}, page_sizes={page_sizes}")
    ...
    for kv_cache_spec_one_worker in kv_cache_specs:
        for layer_name, layer_spec in kv_cache_spec_one_worker.items():
            if layer_name not in merged_kv_cache_specs:
                ...
                if merged_kv_cache_specs[layer_name] != layer_spec:
                    print(f"[DEBUG kv_cache_utils] MISMATCH for layer '{layer_name}':")
                    print(f"[DEBUG kv_cache_utils]   existing: {merged_kv_cache_specs[layer_name]}")
                    print(f"[DEBUG kv_cache_utils]   new:      {layer_spec}")
                assert merged_kv_cache_specs[layer_name] == layer_spec, (
                    f"The KV cache specs for layer '{layer_name}' are different across workers. "
                    f"existing={merged_kv_cache_specs[layer_name]} new={layer_spec}"
                )
